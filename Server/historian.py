...
def usage():
    ...
    resultHtml += ("  -a: show all wakelocks (don't abbreviate system wakelocks)")
    resultHtml += ("  -c: disable drawing of chart")
    resultHtml += ("  -d: debug mode, output debugging info for this program")
    resultHtml += ("  -e TIME: extend billing an extra TIME seconds after each\n"
                   "     wakelock, or until the next wakelock is seen.  Useful for\n"
                   "     accounting for modem power overhead.")
    resultHtml += ("  -h: print this message.")
    resultHtml += ("  -m: generate output that can be embedded in an existing page.\n"
                   "     HTML header and body tags are not outputted.")
    resultHtml += ("  -n [CATEGORY=]PROC: output another row containing only processes\n"
                   "     whose name matches uid of PROC in CATEGORY.\n"
                   "     If CATEGORY is not specified, search in wake_lock_in.")
    resultHtml += ("  -p FILE: analyze FILE containing power data.  Format per\n"
                   "     line: <timestamp in epoch seconds> <amps>")
    resultHtml += ("  -q TIME: quantize data on power row in buckets of TIME\n"
                   "     seconds (default %d)" % getopt_power_quanta)
    resultHtml += ("  -r NAME: report input file name as NAME in HTML.")
    resultHtml += ("  -s PCT: summarize certain useful rows with additional rows\n"
                   "     showing percent time spent over PCT% in each.")
    resultHtml += ("  -t: sort power report by wakelock duration instead of charge")
    resultHtml += ("  -v: synchronize device time before collecting power data")
    resultHtml += ("\n")
    sys.exit(1)
...
def add_emit_event(emit_dict, cat, name, start, end):
    ...
    if end < start:
        resultHtml += ("BUG: end time before start time: %s %s %s<br>" % (name,
                                                                          start,
                                                                          end))
    else:
        if getopt_debug:
            resultHtml += ("Stored emitted event: %s<br>" % str(newevent))
...
class Printer(object):
    ...
    def aggregate_events(self, emit_dict):
        ...
        """
        output_dict = {}
        for cat, events in emit_dict.items():
            output_dict[cat] = []
            start_dict = {}
            for event in events:
                start_time = event[1]
            ...
            for start_time, event_list in start_dict.items():
                if cat == "wifi_suppl":
    ...
    def print_emit_dict(self, cat, emit_dict):
        ...
        for e in emit_dict[cat]:
            if cat == "wake_lock":
                ...
                cat_name = cat
            resultHtml += ("['%s', '%s', %s, %s]," % (cat_name, e[0],
    ...
    def print_highlight_dict(self, highlight_dict):
        ...
        if getopt_highlight_category in highlight_dict:
            for e in highlight_dict[getopt_highlight_category]:
                resultHtml += ("['%s', '%s', %s, %s]," % (catname, e[0],
                                                          timestr_to_jsdate(e[1]),
                                                          timestr_to_jsdate(e[2])))
...
class LegacyFormatConverter(object):
    ...
    def convert(self, input_file):
        ...
        if not self._end_time:
            resultHtml += "cannot find end time"
            sys.exit(1)
...
class BHEmitter(object):
    ...
    def store_event(self, cat, subcat, event_str, event_time, timestr):
        ...
        if getopt_debug:
            resultHtml += ("store_event: %s in %s/%s<br>" % (event_str, cat, subcat))
    ...
    def retrieve_event(self, cat, subcat):
        ...
        if cat in self._in_progress_dict:
            try:
                ...
                if getopt_debug:
                    resultHtml += ("retrieve_event: found %s/%s<br>" % (cat, subcat))
                return (True, result)
            except KeyError:
                pass
        if getopt_debug:
            resultHtml += ("retrieve_event: no match for event %s/%s<br>" % (cat, subcat))
    ...
    def annotate_event_name(self, name):
        ...
        if "*alarm*" in name:
            ...
            if getopt_debug:
                resultHtml += ("annotate_event_name: %s" % name)
        return name
    ...
    def track_event_parallelism_fn(self, start_time, time_this_quanta, time_dict):
        ...
        if start_time in time_dict:
            ...
            time_dict[start_time] = time_this_quanta
        if getopt_debug:
            resultHtml += ("time_dict[%d] now %f added %f" % (start_time,
    ...
    def handle_event(self, event_time, time_str, event_str,
        ...
        """
        if getopt_debug:
            resultHtml += ("<p>handle_event: %s at %s<br>" % (event_str, time_str))
...
class PowerEmitter(object):
    ...
    def get_range_power_fn(self, start_time, time_this_quanta, time_dict):
        ...
        if start_time in self._power_dict:
            ...
            if getopt_debug:
                resultHtml += ("get_range_power: distance %f total time %f "
            ...
            if getopt_debug:
                resultHtml += "get_range_power: no power data available"
            result = 0.0
        return result
    ...
    def bill(self, time_dict):
        ...
        for _, e in enumerate(self._cat_list):
            ...
            if getopt_debug:
                resultHtml += ("billed range %f %f at %fAs to %s<br>" % (start_time, end_time,
                                                                         amps, event_name))
    ...
    def report(self):
        ...
        if report_power:
            ...
            resultHtml += ("<p>Total power: %.3f mAh, avg %.3f" % (mah, avg_ma))
            top_mah = as_to_mah(self._total_top_amps)
            resultHtml += ("<br>Total power above awake "
                           "threshold (%.1fmA): %.3f mAh %.3f As" % (self._TOP_THRESH * 1000,
                                                                     top_mah,
                                                                     self._total_top_amps))
            resultHtml += ("<br>%d samples, %d min<p>" % (self._line_ctr, self._line_ctr / 60))

        if report_power and getopt_bill_extra_secs:
            resultHtml += ("<b>Power seen during each history event, including %d "
                           "seconds after each event:" % getopt_bill_extra_secs)
        elif report_power:
            resultHtml += "<b>Power seen during each history event:"
        else:
            resultHtml += "<b>Event summary:"
        resultHtml += "</b><br><pre>"

        report_list = []
        total_mah = 0.0
        total_count = 0
        for _, v in self._synopsis_dict.items():
            total_mah += v.mah
        ...
        for i in report_list:
            resultHtml += i[1]
        resultHtml += ("total: %.3f mAh, %d events" % (total_mah, total_count))
        resultHtml += "</pre>\n"
...
def main(dataRaw):
    ...
    for line in dataRaw:
        ...
        if time_delta_s < 0:
            # resultHtml += ("Warning: time went backwards: %s" % line)
            continue
    ...
    if not on_mode:
        resultHtml += "Battery history not present in bugreport."
        return
    ...
    header = "Battery Historian analysis"
    resultHtml += ("<title>" + header + "</title>")
    if overflowed:
        resultHtml += ('<font size="5" color="red">Warning: History overflowed at %s, '
                       'many events may be missing.</font>' %
                       time_float_to_human(data_stop_time, True))
    resultHtml += ("<p>" + header + "</p>")
    ...
    if not getopt_disable_chart_drawing:
        resultHtml += "google.setOnLoadCallback(drawChart);\n"
...
""")
    ...
    resultHtml += "<div id=\"chart\">"
    resultHtml += "</div>"
    resultHtml += ("<pre>(Local time %s - %s, %dm elapsed)</pre>"
